 ...
 * 
 */
export default class FguiResLoaderManager {
    private static _instance: FguiResLoaderManager;
    public static get instance(): FguiResLoaderManager {
        if (!this._instance)
            this._instance = new FguiResLoaderManager();
     ...
     * 混合加载 
     * @param resKey //fgui资源
     * @param otherLoadData //其它资源 
     * @param complete 
     * @param progress 
     * @param priority 
     */
    public load(resKey:string|string[], otherLoadData: any, complete?: Laya.Handler, progress?: Laya.Handler, priority?: number) {
        if (priority) {
